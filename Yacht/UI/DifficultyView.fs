module Yacht.UI.DifficultyView
...
let create (dispatch: Msg -> unit) : View =
  ...
  let items =
    ObservableCollection<string> [| "Easy"; "Intermediate"; "Hard"; "Back" |]
  ...
  list.Accepting.Add(fun _ ->
    ...
    match idx with
    | 0 -> dispatch (SelectDifficulty Easy)
    | 1 -> dispatch (SelectDifficulty Intermediate)
    | 2 -> dispatch (SelectDifficulty Hard)
    | 3 -> dispatch BackToMenu
    | _ -> ())

  list |> frame.Add |> ignore
  frame :> View
